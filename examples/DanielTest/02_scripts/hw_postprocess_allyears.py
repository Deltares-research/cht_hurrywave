...
output_dir = os.path.join(base_path, "04_modelruns" , "YearSims", "01_postprocess", "Allyears")
...
def plot_direction_error_histogram(df_directions, save_path, title, data_type):
    ...
    for station, stats in df_directions.items():
        ...
        plt.savefig(os.path.join(outpath, f"{title.replace(' ', '_')}_hist_{station}.png"))
        plt.close()
        ...
        for station in model_dict.keys():
            for model_var in variable_list:
                ...
                try:
                    ...
                    plot_bootstrap_means_and_stdevs(obs_dict[station][buoy_var], model_dict[station][model_var], station, model_var,stats_dir, block_size=block_size)
