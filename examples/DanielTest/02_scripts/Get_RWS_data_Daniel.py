...
th0_locations = wave_data_locs[wave_data_locs['Grootheid.Code'].str.contains('Th0')]
...
for _, location in th0_locations.iterrows():
    get_data(location, 'th0' ,start_date, end_date,output_path,dict_measurements, overwrite=True)
...
def write_to_csv(dict_measurements, outpath, station):
    ...
    required_vars = ['hm0', 't13', 'tmax', 'th0']
    ...
    for var in required_vars:
        if var in dict_measurements[station] and dict_measurements[station][var] is not None and len(dict_measurements[station][var]) > 0:
            ...
            if index is None:
                ...
                nan_array_length = nan_length
            else:
                nan_array_length = len(index)
            df_nan = pd.DataFrame({var: [np.nan]*nan_array_length}, index=index)
    ...
    # Remove duplicate indices before reindexing and interpolate each variable
    for var in required_vars:
        dfs[var] = dfs[var][~dfs[var].index.duplicated(keep='first')]
        dfs[var] = dfs[var].reindex(hourly_index).interpolate(method='time')

    # Combine all variables into one dataframe
    combined_df = pd.concat([dfs['hm0']['hm0'], dfs['t13']['t13'], dfs['tmax']['tmax'], dfs['th0']['th0']], axis=1)
