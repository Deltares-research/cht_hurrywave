...
output_dir = os.path.join(base_path, "04_modelruns" , "YearSims", "01_postprocess", "Withoutliers")
# output_dir = os.path.join(base_path, "04_modelruns" , "YearSims", "01_postprocess", "TEST")
...
def process_era5_station_data(ERA5_data_path, variables, years, station_names, xs, ys,
    ...
    """
    Extract ERA5 data into a dict-of-dicts:
        {station: {var: array, 'time': array, 'station_x': float, 'station_y': float}}

    Parameters
    ----------
    ERA5_data_path : str
        Base directory with ERA5 NetCDF files (organized per variable).
    variables : dict
        Map of short variable names -> subdirectory and filename pattern
        e.g. {'swh': 'significant_height_of_combined_wind_waves_and_swell',
              'pp1d': 'peak_wave_period'}
    years : list[int]
        Years to process
    ...
    tstart_dt, tstop_dt : np.datetime64
        Time slicing window
    area : [N, W, S, E]
        Spatial cropping area
    save : bool
        If True, save each station as .npz
    outdir : str
    ...
    for y in years:
        ...
        # Load each variable for this year
        for varname, subdir in variables.items():
            fname = os.path.join(
                ERA5_data_path,
                subdir,
                f'global_{subdir}_{y}.nc'
            )
            ds = xr.open_dataset(fname)
        ...
        # Now extract stations for this year
        ...
        for i, (station, lon, lat) in enumerate(zip(station_names, xs, ys)):
            ...
            for varname, ds in year_datasets.items():
                ...
                point = ds[varname].sel(latitude=lat, longitude=lon, method='nearest')
                point = point.squeeze(drop=True)   # ensures only 'valid_time' left
                ERA5_data_per_station[station][varname].append(point.values)
                # print(varname, point.shape, point.dims)
    ...
    for station in ERA5_data_per_station:
        ...
        if 'time' in ERA5_data_per_station[station]:
            ...
            print("Sorted for station:", station)
    ...
    if save and outdir is not None:
        os.makedirs(outdir, exist_ok=True)
        for station, data in ERA5_data_per_station.items():
            outpath = os.path.join(outdir, f'{station}.npz')
            np.savez(outpath, **data)
            print(f'  ✓ Saved {station}.npz with {len(data["time"])} timesteps.')
